...
newUHashStructure(IntT typeHT, Int32T hashTableSize, IntT bucketVectorLength,
  ...
  case HT_STATISTICS:
    ...
    FAILIF(NULL == (uhash->hashTable.linkHashTable = (LinkPackedGBucketT **)
                        MALLOC(hashTableSize * sizeof(LinkPackedGBucketT *))));
    FAILIF(NULL == (uhash->chainSizes =
                        (IntT *)(MALLOC(hashTableSize * sizeof(IntT)))));
    FAILIF(NULL == (uhash->bucketPoints.pointsList = (PointsListEntryT *)MALLOC(
                        hashTableSize * sizeof(PointsListEntryT))));
    for (Int32T i = 0; i < hashTableSize; i++) {
      uhash->chainSizes[i] = CHAIN_INIT_SIZE;
      FAILIF(
          NULL ==
              (uhash->hashTable.linkHashTable[i] = (LinkPackedGBucketT *)MALLOC(
                   uhash->chainSizes[i] * sizeof(LinkPackedGBucketT))) &&
  ...
  case HT_PACKED:
    ...
    //     (PackedGBucketT**)MALLOC(hashTableSize * sizeof(PackedGBucketT*))));
    //     FAILIF(NULL == (uhash->chainSizes = (IntT*)(MALLOC(hashTableSize *
    //     sizeof(IntT))))); FAILIF(NULL == (uhash->bucketPoints.pointsArray =
    //     (PPointT*)MALLOC(hashTableSize * sizeof(PPointT)))); totalN = 0; //
    //     total number of points hashed so far. for(Int32T i = 0; i <
    //     hashTableSize; i++){
    ...
    // modelHT->bucketPoints.pointsList[p].point; 	  totalN++; 	  count++; 	  p =
  ...
  case HT_HYBRID_CHAINS:
    ...
    FAILIF(NULL == (uhash->hashTable.hybridHashTable = (PHybridChainEntryT *)
                        MALLOC(hashTableSize * sizeof(PHybridChainEntryT))));
    FAILIF(NULL == (uhash->hybridChainsStorage = (HybridChainEntryT *)MALLOC(
    ...
    // (the entries of <uhash->hybridChainsStorage> are filled from start and
    // from end: at the beginning we fill the normal buckets with their points;
    // at the end we fill the "overflow" points of buckets (additional points of
    // buckets that have more than MAX_NONOVERFLOW_POINTS_PER_BUCKET points).
    ...
    for (Int32T i = 0; i < hashTableSize; i++) {
      ...
      while (bucket != NULL) {
        ...
        indexInStorage++;
        uhash->hybridChainsStorage[indexInStorage].point.isLastBucket =
            (bucket->nextGBucketInChain == NULL ? 1 : 0);
        uhash->hybridChainsStorage[indexInStorage].point.bucketLength =
            (nPointsInBucket <= MAX_NONOVERFLOW_POINTS_PER_BUCKET
                 ? nPointsInBucket
                 : 0); // 0 means there are "overflow" points
        uhash->hybridChainsStorage[indexInStorage].point.isLastPoint =
            (nPointsInBucket == 1 ? 1 : 0);
        uhash->hybridChainsStorage[indexInStorage].point.pointIndex =
            bucket->firstEntry.pointIndex;
        indexInStorage++;

        // Store all other points in the storage
        Uns32T currentIndex =
            indexInStorage; // index where the "current" point will be stored.
        Uns32T nOverflow = 0;
        ...
        if (nPointsInBucket <= MAX_NONOVERFLOW_POINTS_PER_BUCKET) {
          ...
          Uns32T value = overflowStart -
                         (currentIndex - 1 + MAX_NONOVERFLOW_POINTS_PER_BUCKET);
          for (IntT j = 0; j < N_FIELDS_PER_INDEX_OF_OVERFLOW; j++) {
            uhash->hybridChainsStorage[currentIndex + j].point.bucketLength =
          ...
          indexInStorage =
              indexInStorage + MAX_NONOVERFLOW_POINTS_PER_BUCKET - 1;
          ASSERT(indexInStorage <= lastIndexInSt + 1);

          // FAILIFWR(nPointsInBucket > MAX_NONOVERFLOW_POINTS_PER_BUCKET, "Too
          // many points in a bucket -- feature not implemented yet. Try to
          // lower N_BITS_PER_POINT_INDEX as much as possible.");// TODO: not
          // implemented yet
        }

        bucketEntry = bucket->firstEntry.nextEntry;
        while (bucketEntry != NULL) {
          uhash->hybridChainsStorage[currentIndex].point.pointIndex =
          ...
          if (currentIndex == indexInStorage &&
              nPointsInBucket > MAX_NONOVERFLOW_POINTS_PER_BUCKET) {
            // finished the normal alloted space -> going to the space reserved
            // at end of the table.
        ...
        // ASSERT((uhash->hashTable.hybridHashTable[i] + 1)->point.bucketLength
        // > 0);
      }
    }
    ASSERT(indexInStorage == lastIndexInSt + 1);
...
void optimizeUHashStructure(PUHashStructureT uhash,
  ...
  for (Int32T i = 0; i < uhash->hashTableSize; i++) {
    for (IntT j = 0;
         j < uhash->chainSizes[i] &&
         uhash->hashTable.linkHashTable[i][j].indexStart != INDEX_START_EMPTY;
         j++) {
...
inline Uns32T combinePrecomputedHashes(Uns32T *firstBucketVector,
  ...
  //   } else {
  //     CR_ASSERT(nBucketVectorPieces == 2); // each of the <g> functions is a
  //     pair of 2 <u> functions.
...
void addBucketEntry(PUHashStructureT uhash, IntT nBucketVectorPieces,
  ...
  if (!USE_PRECOMPUTED_HASHES) {
    ...
    hIndex = computeUHashFunction(uhash->mainHashA, tempVector,
                                  nBucketVectorPieces, uhash->hashedDataLength,
                                  uhash->prime, uhash->hashTableSize);
  ...
  case HT_LINKED_LIST:
    ...
    if (p == NULL) {
      ...
      uhash->hashTable.llHashTable[hIndex] = newGBucket(
          uhash, control1, pointIndex, uhash->hashTable.llHashTable[hIndex]);
    } else {
  ...
  case HT_PACKED:
    ...
    //     uhash->hashTable.packedHashTable[hIndex][i].nPointsInBucket > 0;
    //     i++){
    //       if (uhash->hashTable.packedHashTable[hIndex][i].controlValue1 ==
    //       control1){
    // 	break;
    //       }
    //     }
  ...
  case HT_STATISTICS:
    ...
    //     found = FALSE;
    //     for(j = 0; j < uhash->chainSizes[hIndex] &&
...
GeneralizedPGBucket getGBucket(PUHashStructureT uhash, IntT nBucketVectorPieces,
  ...
  case HT_STATISTICS:
    ...
    for (IntT j = 0; j < uhash->chainSizes[hIndex] &&
                     uhash->hashTable.linkHashTable[hIndex][j].indexStart !=
                         INDEX_START_EMPTY;
         j++) {
