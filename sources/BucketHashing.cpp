...
// expected to be NULL.
inline PGBucketT newGBucket(PUHashStructureT uhash, Uns32T control1,
                            /*PPointT point, */ Int32T pointIndex,
                            PGBucketT nextGBucket) {
    PGBucketT bucket;
...
inline void addPointToGBucket(PUHashStructureT uhash,
                              PGBucketT bucket /*, PPointT point*/,
                              Int32T pointIndex) {
    ASSERT(bucket != NULL);
    ASSERT(uhash != NULL);
    ...
    if (uhash->unusedPBucketEntrys != NULL) {
        ...
        FAILIF(NULL ==
               (bucketEntry = (PBucketEntryT)MALLOC(sizeof(BucketEntryT))));
        nAllocatedBEntries++;
    }
...
PUHashStructureT newUHashStructure(IntT typeHT, Int32T hashTableSize,
    ...
    switch (typeHT) {
        case HT_LINKED_LIST:
            FAILIF(NULL == (uhash->hashTable.llHashTable = (PGBucketT *)MALLOC(
                                hashTableSize * sizeof(PGBucketT))));
            uhash->chainSizes = NULL;
        ...
        case HT_STATISTICS:
            ...
            FAILIF(NULL ==
                   (uhash->hashTable.linkHashTable = (LinkPackedGBucketT **)
                        MALLOC(hashTableSize * sizeof(LinkPackedGBucketT *))));
            FAILIF(NULL == (uhash->chainSizes = (IntT *)(MALLOC(
                                hashTableSize * sizeof(IntT)))));
            FAILIF(NULL ==
                   (uhash->bucketPoints.pointsList = (PointsListEntryT *)MALLOC(
                        hashTableSize * sizeof(PointsListEntryT))));
            for (Int32T i = 0; i < hashTableSize; i++) {
                uhash->chainSizes[i] = CHAIN_INIT_SIZE;
                FAILIF(NULL == (uhash->hashTable.linkHashTable[i] =
                                    (LinkPackedGBucketT *)MALLOC(
                                        uhash->chainSizes[i] *
                                        sizeof(LinkPackedGBucketT))) &&
                       (uhash->chainSizes[i] > 0));
                if (uhash->chainSizes[i] > 0) {
                    // the first bucket is empty
                    uhash->hashTable.linkHashTable[i][0].indexStart =
                        INDEX_START_EMPTY;
                }
            }
            break;
        case HT_PACKED:
            ...
            //     FAILIF(NULL == (uhash->hashTable.packedHashTable =
            //     (PackedGBucketT**)MALLOC(hashTableSize *
            //     sizeof(PackedGBucketT*)))); FAILIF(NULL == (uhash->chainSizes
            //     = (IntT*)(MALLOC(hashTableSize * sizeof(IntT)))));
            //     FAILIF(NULL == (uhash->bucketPoints.pointsArray =
            //     (PPointT*)MALLOC(hashTableSize * sizeof(PPointT)))); totalN =
            //     0; // total number of points hashed so far. for(Int32T i = 0;
            //     i < hashTableSize; i++){
            ...
            //       IntT j;
            //       for(j = 0; j < modelHT->chainSizes[i] &&
            //       modelHT->hashTable.linkHashTable[i][j].indexStart !=
            //       INDEX_START_EMPTY; j++)
            // 	;
            ...
            //       FAILIF(NULL == (uhash->hashTable.packedHashTable[i] =
            //       (PackedGBucketT*)MALLOC(uhash->chainSizes[i] *
            //       sizeof(PackedGBucketT))));

            //       // copy each bucket in the chain:
            //       for(j = 0; j < uhash->chainSizes[i]; j++){
            // 	// "general" info for the bucket
            // 	uhash->hashTable.packedHashTable[i][j].controlValue1 =
            // modelHT->hashTable.linkHashTable[i][j].controlValue1;
            ...
            // 	  uhash->bucketPoints.pointsArray[totalN] =
            // modelHT->bucketPoints.pointsList[p].point; 	  totalN++;
            // count++; 	  p =
            // modelHT->bucketPoints.pointsList[p].nextPoint;
            // 	}
        ...
        case HT_HYBRID_CHAINS:
            ...
            FAILIF(NULL ==
                   (uhash->hashTable.hybridHashTable = (PHybridChainEntryT *)
                        MALLOC(hashTableSize * sizeof(PHybridChainEntryT))));
            FAILIF(NULL ==
                   (uhash->hybridChainsStorage = (HybridChainEntryT *)MALLOC(
                        (modelHT->nHashedPoints + modelHT->nHashedBuckets) *
                        sizeof(HybridChainEntryT))));

            // the index of the first unoccupied entry in
            // <uhash->hybridChainsStorage>.
            indexInStorage = 0;

            // the index of the last unoccupied entry in
            // <uhash->hybridChainsStorage> (the entries of
            // <uhash->hybridChainsStorage> are filled from start and from end:
            // at the beginning we fill the normal buckets with their points; at
            // the end we fill the "overflow" points of buckets (additional
            // points of buckets that have more than
            // MAX_NONOVERFLOW_POINTS_PER_BUCKET points).
            lastIndexInSt =
                modelHT->nHashedPoints + modelHT->nHashedBuckets - 1;

            for (Int32T i = 0; i < hashTableSize; i++) {
                PGBucketT bucket = modelHT->hashTable.llHashTable[i];
                if (bucket != NULL) {
                    uhash->hashTable.hybridHashTable[i] =
                        uhash->hybridChainsStorage +
                        indexInStorage;  // the position where the bucket starts
                } else {
                ...
                while (bucket != NULL) {
                    ...
                    uhash->hybridChainsStorage[indexInStorage].controlValue1 =
                        bucket->controlValue1;
                    indexInStorage++;
                    uhash->hybridChainsStorage[indexInStorage]
                        .point.isLastBucket =
                        (bucket->nextGBucketInChain == NULL ? 1 : 0);
                    uhash->hybridChainsStorage[indexInStorage]
                        .point.bucketLength =
                        (nPointsInBucket <= MAX_NONOVERFLOW_POINTS_PER_BUCKET
                             ? nPointsInBucket
                             : 0);  // 0 means there are "overflow" points
                    uhash->hybridChainsStorage[indexInStorage]
                        .point.isLastPoint = (nPointsInBucket == 1 ? 1 : 0);
                    uhash->hybridChainsStorage[indexInStorage]
                        .point.pointIndex = bucket->firstEntry.pointIndex;
                    indexInStorage++;

                    // Store all other points in the storage
                    Uns32T currentIndex =
                        indexInStorage;  // index where the "current" point will
                                         // be stored.
                    Uns32T nOverflow = 0;
                    ...
                    if (nPointsInBucket <= MAX_NONOVERFLOW_POINTS_PER_BUCKET) {
                        ...
                        // bucket too large.
                        // store the overflow points at the end of the array
                        // <uhash->hybridChainsStorage>.
                        nOverflow =
                            nPointsInBucket - MAX_NONOVERFLOW_POINTS_PER_BUCKET;
                        ...
                        // specify the offset of the start of overflow points in
                        // the fields <bucketLength> of points 2, 3, ... of the
                        // space immediately after the bucket.
                        Uns32T value =
                            overflowStart - (currentIndex - 1 +
                                             MAX_NONOVERFLOW_POINTS_PER_BUCKET);
                        for (IntT j = 0; j < N_FIELDS_PER_INDEX_OF_OVERFLOW;
                             j++) {
                            uhash->hybridChainsStorage[currentIndex + j]
                                .point.bucketLength =
                                value & ((1U << N_BITS_FOR_BUCKET_LENGTH) - 1);
                            value = value >> N_BITS_FOR_BUCKET_LENGTH;
                        }

                        // update <indexInStorage>
                        indexInStorage = indexInStorage +
                                         MAX_NONOVERFLOW_POINTS_PER_BUCKET - 1;
                        ASSERT(indexInStorage <= lastIndexInSt + 1);

                        // FAILIFWR(nPointsInBucket >
                        // MAX_NONOVERFLOW_POINTS_PER_BUCKET, "Too many points
                        // in a bucket -- feature not implemented yet. Try to
                        // lower N_BITS_PER_POINT_INDEX as much as
                        // possible.");// TODO: not implemented yet
                    }

                    bucketEntry = bucket->firstEntry.nextEntry;
                    while (bucketEntry != NULL) {
                        uhash->hybridChainsStorage[currentIndex]
                            .point.pointIndex = bucketEntry->pointIndex;
                        uhash->hybridChainsStorage[currentIndex]
                            .point.isLastPoint = 0;
                        bucketEntry = bucketEntry->nextEntry;

                        currentIndex++;
                        if (currentIndex == indexInStorage &&
                            nPointsInBucket >
                                MAX_NONOVERFLOW_POINTS_PER_BUCKET) {
                            // finished the normal alloted space -> going to the
                            // space reserved at end of the table.
                            currentIndex = overflowStart;
                        }
                    }

                    // set the <isLastBucket> field of the last point = 1.
                    uhash->hybridChainsStorage[currentIndex - 1]
                        .point.isLastPoint = 1;

                    bucket = bucket->nextGBucketInChain;
                    // ASSERT((uhash->hashTable.hybridHashTable[i] +
                    // 1)->point.bucketLength > 0);
                }
            }
            ASSERT(indexInStorage == lastIndexInSt + 1);
    ...
    if (!useExternalUHFs) {
        FAILIF(NULL == (uhash->mainHashA = (Uns32T *)MALLOC(
                            uhash->hashedDataLength * sizeof(Uns32T))));
    ...
    if (!useExternalUHFs) {
        FAILIF(NULL == (uhash->controlHash1 = (Uns32T *)MALLOC(
                            uhash->hashedDataLength * sizeof(Uns32T))));
        for (IntT i = 0; i < uhash->hashedDataLength; i++) {
        ...
        case HT_LINKED_LIST:
            for (Int32T i = 0; i < uhash->hashTableSize; i++) {
                ...
                while (bucket != NULL) {
                    ...
                    PBucketEntryT bucketEntry =
                        tempBucket->firstEntry.nextEntry;
                    while (bucketEntry != NULL) {
...
void optimizeUHashStructure(PUHashStructureT uhash,
    ...
    if (auxPtsList == NULL) {
        FAILIF(NULL == (auxPtsList = (PointsListEntryT *)MALLOC(
                            uhash->hashTableSize * sizeof(PointsListEntryT))));
    }

    Int32T newP = 0;
    for (Int32T i = 0; i < uhash->hashTableSize; i++) {
        for (IntT j = 0; j < uhash->chainSizes[i] &&
                         uhash->hashTable.linkHashTable[i][j].indexStart !=
                             INDEX_START_EMPTY;
             j++) {
            Int32T p = uhash->hashTable.linkHashTable[i][j].indexStart;
            uhash->hashTable.linkHashTable[i][j].indexStart = newP;
            while (p != INDEX_START_EMPTY) {
                auxPtsList[newP].point =
                    uhash->bucketPoints.pointsList[p].point;
...
inline Uns32T computeBlockProductModDefaultPrime(Uns32T *a, Uns32T *(b[]),
                                                 IntT nBPieces, IntT size) {
    LongUns64T h = 0;
    IntT j = 0;
    IntT bPiece = 0;
...
inline Uns32T computeUHashFunction(Uns32T *rndVector, Uns32T *(data[]),
                                   IntT nDataPieces, IntT size, Uns32T prime,
...
inline Uns32T combinePrecomputedHashes(Uns32T *firstBucketVector,
    ...
    //   } else {
    //     CR_ASSERT(nBucketVectorPieces == 2); // each of the <g> functions is
    //     a pair of 2 <u> functions.
    //     //CR_ASSERT(bucketVector[2] != NULL);
    //     //CR_ASSERT(bucketVector[3] != NULL);
    //     LongUns64T r = (LongUns64T)(bucketVector[2][uhfIndex]) +
    //     (LongUns64T)(bucketVector[3][uhfIndex + UHF_NUMBER_OF_HASHES]); if (r
    //     >= UH_PRIME_DEFAULT) {
    //       r -= UH_PRIME_DEFAULT;
    //     }
    ...
    if (nBucketVectorPieces == 1) {
        ...
        CR_ASSERT(
            nBucketVectorPieces ==
            2);  // each of the <g> functions is a pair of 2 <u> functions.
        // CR_ASSERT(bucketVector[2] != NULL);
        // CR_ASSERT(bucketVector[3] != NULL);
        LongUns64T r =
            (LongUns64T)(firstBucketVector[uhfIndex]) +
            (LongUns64T)(secondBucketVector[uhfIndex + UHF_NUMBER_OF_HASHES]);
...
void addBucketEntry(PUHashStructureT uhash, IntT nBucketVectorPieces,
                    ...
                    Uns32T secondBucketVector[] /*, PPointT point*/,
                    Int32T pointIndex) {
    ...
    if (!USE_PRECOMPUTED_HASHES) {
        ...
        hIndex = computeUHashFunction(
            uhash->mainHashA, tempVector, nBucketVectorPieces,
            uhash->hashedDataLength, uhash->prime, uhash->hashTableSize);
        control1 = computeBlockProductModDefaultPrime(
            uhash->controlHash1, tempVector, nBucketVectorPieces,
            uhash->hashedDataLength);
    } else {
        ...
        hIndex = combinePrecomputedHashes(firstBucketVector, secondBucketVector,
                                          nBucketVectorPieces, UHF_MAIN_INDEX) %
                 uhash->hashTableSize;
        control1 =
            combinePrecomputedHashes(firstBucketVector, secondBucketVector,
                                     nBucketVectorPieces, UHF_CONTROL1_INDEX);
    }
    ...
    switch (uhash->typeHT) {
        case HT_LINKED_LIST:
            ...
            while (p != NULL && (p->controlValue1 != control1)) {
            ...
            if (p == NULL) {
                ...
                uhash->hashTable.llHashTable[hIndex] =
                    newGBucket(uhash, control1, pointIndex,
                               uhash->hashTable.llHashTable[hIndex]);
            } else {
        ...
        case HT_PACKED:
            ...
            //     IntT i;
            //     for(i = 0; i < uhash->chainSizes[hIndex] &&
            //     uhash->hashTable.packedHashTable[hIndex][i].nPointsInBucket >
            //     0; i++){
            //       if
            //       (uhash->hashTable.packedHashTable[hIndex][i].controlValue1
            //       == control1){
            // 	break;
            //       }
            //     }
        ...
        case HT_STATISTICS:
            ASSERT(FALSE);
            //     uhash->bucketPoints.pointsList[uhash->nHashedPoints].point =
            //     point; found = FALSE; for(j = 0; j <
            //     uhash->chainSizes[hIndex] &&
            //     uhash->hashTable.linkHashTable[hIndex][j].indexStart !=
            //     INDEX_START_EMPTY; j++){
            //       if (uhash->hashTable.linkHashTable[hIndex][j].controlValue1
            //       == control1){
            // 	found = TRUE;
            // 	break;
            //       }
            ...
            // 	  uhash->chainSizes[hIndex] = CEIL(uhash->chainSizes[hIndex] *
            // CHAIN_RESIZE_RATIO); 	}else{ 	  uhash->chainSizes[hIndex] = 1;
            // 	}
            // 	uhash->hashTable.linkHashTable[hIndex] =
            // (LinkPackedGBucketT*)REALLOC(uhash->hashTable.linkHashTable[hIndex],
            // uhash->chainSizes[hIndex] * sizeof(LinkPackedGBucketT));
            // 	uhash->hashTable.linkHashTable[hIndex][j].controlValue1 =
            // control1;
            //       }
            //       uhash->hashTable.linkHashTable[hIndex][j].controlValue1 =
            //       control1;
            //       uhash->hashTable.linkHashTable[hIndex][j].indexStart =
            //       INDEX_START_EMPTY; uhash->nHashedBuckets++; if (j + 1 <
            //       uhash->chainSizes[hIndex]){
            // 	uhash->hashTable.linkHashTable[hIndex][j + 1].indexStart =
            // INDEX_START_EMPTY;
            //       }
            //     }
            ...
            //     uhash->hashTable.linkHashTable[hIndex][j].indexStart =
            //     uhash->nHashedPoints;
            //     uhash->bucketPoints.pointsList[uhash->nHashedPoints].nextPoint
            //     = temp;
    ...
    switch (uhash->typeHT) {
        ...
        case HT_STATISTICS:
            ...
            for (IntT j = 0;
                ...
                if (uhash->hashTable.linkHashTable[hIndex][j].controlValue1 ==
                    control1) {
                    result.linkGBucket =
                        &(uhash->hashTable.linkHashTable[hIndex][j]);
                    return result;
                }
            }
        ...
        case HT_PACKED:
            ...
            for (IntT j = 0; j < uhash->chainSizes[hIndex]; j++) {
                if (uhash->hashTable.packedHashTable[hIndex][j].controlValue1 ==
                    control1) {
                    result.packedGBucket =
                        &(uhash->hashTable.packedHashTable[hIndex][j]);
                    return result;
                }
            }
...
void precomputeUHFsForULSH(PUHashStructureT uhash, Uns32T *uVector, IntT length,
    ...
    if (length == uhash->hashedDataLength) {
        result[UHF_MAIN_INDEX] =
            computeProductModDefaultPrime(uhash->mainHashA, uVector, length);
        result[UHF_CONTROL1_INDEX] =
            computeProductModDefaultPrime(uhash->controlHash1, uVector, length);
    } else {
        ASSERT(
            2 * length ==
            uhash->hashedDataLength);  // the length is 1/2 of the bucket length
        result[UHF_MAIN_INDEX] =
            computeProductModDefaultPrime(uhash->mainHashA, uVector, length);
        result[UHF_CONTROL1_INDEX] =
            computeProductModDefaultPrime(uhash->controlHash1, uVector, length);
        result[UHF_MAIN_INDEX + UHF_NUMBER_OF_HASHES] =
            computeProductModDefaultPrime(uhash->mainHashA + length, uVector,
                                          length);
        result[UHF_CONTROL1_INDEX + UHF_NUMBER_OF_HASHES] =
            computeProductModDefaultPrime(uhash->controlHash1 + length, uVector,
                                          length);
    }
}
