...
void usage(char *programName) {
    printf(
        "Usage: %s dimension max_size #_of_points R probability_of_success\n",
        programName);
}
...
int main(int nargs, char **args) {
    ...
    // RealT R = 2.0 * M * POW((RealT)nExpNNs / (RealT)nPoints /
    // vol_d_const, 1.0/(RealT)dimension) / SQRT(M_PI) * SQRT((RealT)dimension
    // / 2.0 / M_E);
    ...
    //   RealT R = 2.0 * M * POW((RealT)nExpNNs / (RealT)nPoints /
    //   vol_d_const, 1.0/(RealT)dimension);

    // printf("%Lf %Lf \n", POW(R/2.0/M, dimension), (RealT)nExpNNs /
    // (RealT)nPoints / vol_d_const);
