...
initializePRNearNeighborFields(RNNParametersT algParameters,
  ...
  FAILIF(NULL == (nnStruct->precomputedHashesOfULSHs = (Uns32T **)MALLOC(
                      nnStruct->nHFTuples * sizeof(Uns32T *))));
  for (IntT i = 0; i < nnStruct->nHFTuples; i++) {
    FAILIF(NULL == (nnStruct->precomputedHashesOfULSHs[i] = (Uns32T *)MALLOC(
...
PRNearNeighborStructT initLSH_WithDataSet(RNNParametersT algParameters,
  ...
  // Initialize the counters for defining the pair of <u> functions used for <g>
  // functions.
  IntT firstUComp = 0;
  IntT secondUComp = 1;
  for (IntT i = 0; i < nnStruct->parameterL; i++) {
    ...
    for (IntT p = 0; p < nPoints; p++) {
      ...
      if (!nnStruct->useUfunctions) {
        ...
        addBucketEntry(modelHT, 2, precomputedHashesOfULSHs[firstUComp][p],
    ...
    // copy the model HT into the actual (packed) HT. copy the uhash function
    // too.
    nnStruct->hashedBuckets[i] =
        newUHashStructure(algParameters.typeHT, nPoints, nnStruct->parameterK,
                          TRUE, mainHashA, controlHash1, modelHT);
  ...
  freeUHashStructure(modelHT,
                     FALSE); // do not free the uhash functions since they are
                             // used by nnStruct->hashedBuckets[i]
...
inline void computeULSH(PRNearNeighborStructT nnStruct, IntT gNumber,
  ...
  for (IntT i = 0; i < nnStruct->hfTuplesLength; i++) {
    ...
    vectorValue[i] =
        (Uns32T)(FLOOR_INT32((value + nnStruct->lshFunctions[gNumber][i].b) /
...
Int32T getNearNeighborsFromPRNearNeighborStruct(PRNearNeighborStructT nnStruct,
  ...
  for (IntT i = 0; i < nnStruct->parameterL; i++) {
    ...
    case HT_LINKED_LIST:
      ...
      if (bucket != NULL) {
        // circle through the bucket and add to <result> the points that are
        // near.
        ...
        while (bucketEntry != NULL) {
          ...
          if (isDistanceSqrLeq(nnStruct->dimension, point, candidatePoint,
    ...
    case HT_STATISTICS:
      ...
      // nnStruct->hashedBuckets[i]->bucketPoints.pointsList; 	position =
      // gbucket.linkGBucket->indexStart;
      // 	// circle through the bucket and add to <result> the points that
      // are near. 	while (position != INDEX_START_EMPTY){ 	  PPointT candidatePoint
      // = pointsList[position].point; 	  if (isDistanceSqrLeq(nnStruct->dimension,
      // point, candidatePoint, nnStruct->parameterR2) &&
      // nnStruct->reportingResult){ 	    if
      // (nnStruct->nearPoints[candidatePoint->index] == FALSE) {
      // 	      // a new R-NN point was found (not yet in <result>).
      // 	      if (nNeighbors >= resultSize){
      // 		// run out of space => resize the <result> array.
      // 		resultSize = 2 * resultSize;
      // 		result = (PPointT*)REALLOC(result, resultSize *
      // sizeof(PPointT));
      // 	      }
      // 	      result[nNeighbors] = candidatePoint;
      // 	      nNeighbors++;
      // 	      nnStruct->nearPoints[candidatePoint->index] = TRUE; // do
      // not include more points with the same index
      // 	    }
      // 	  }
      // 	  // Int32T oldP = position;
      // 	  position = pointsList[position].nextPoint;
      // 	  // ASSERT(position == INDEX_START_EMPTY || position == oldP +
      // 1);
      // 	}
      //       }
      break;
      ...
      if (gbucket.hybridGBucket != NULL) {
        ...
        if (hybridPoint->point.bucketLength == 0) {
          ...
          for (IntT j = 0; j < N_FIELDS_PER_INDEX_OF_OVERFLOW; j++) {
            offset += ((Uns32T)((hybridPoint + 1 + j)->point.bucketLength)
        ...
        while (!done) {
          ...
          if (nnStruct->markedPoints[candidatePIndex] == FALSE) {
            ...
            if (isDistanceSqrLeq(nnStruct->dimension, point, candidatePoint,
              ...
              // TIMEV_END(timeResultStoring);
              // nnStruct->markedPointsIndeces[nMarkedPoints] = candidatePIndex;
              // nnStruct->markedPoints[candidatePIndex] = TRUE; // do not
              // include more points with the same index nMarkedPoints++;
              // }
            }
          } else {
