...
void initRandom() { FAILIF(NULL == initstate(2, rngState, 256)); }
...
IntT genRandomInt(IntT rangeStart, IntT rangeEnd) {
    ...
    IntT r;
    r = rangeStart +
        (IntT)((rangeEnd - rangeStart + 1.0) * random() / (RAND_MAX + 1.0));
...
Uns32T genRandomUns32(Uns32T rangeStart, Uns32T rangeEnd) {
    ...
    if (RAND_MAX >= rangeEnd - rangeStart) {
        r = rangeStart + (Uns32T)((rangeEnd - rangeStart + 1.0) * random() /
                                  (RAND_MAX + 1.0));
    } else {
        r = rangeStart +
            (Uns32T)((rangeEnd - rangeStart + 1.0) *
                     ((LongUns64T)random() * ((LongUns64T)RAND_MAX + 1) +
                      (LongUns64T)random()) /
                     ((LongUns64T)RAND_MAX * ((LongUns64T)RAND_MAX + 1) +
                      (LongUns64T)RAND_MAX + 1.0));
    }
    ASSERT(r >= rangeStart && r <= rangeEnd);
    return r;
}
...
RealT genUniformRandom(RealT rangeStart, RealT rangeEnd) {
    ...
    RealT r;
    r = rangeStart +
        ((rangeEnd - rangeStart) * (RealT)random() / (RealT)RAND_MAX);
    ASSERT(r >= rangeStart && r <= rangeEnd);
    return r;
}
