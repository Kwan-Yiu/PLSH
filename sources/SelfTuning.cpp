...
void sortQueryPointsByRadii(IntT dimension, Int32T nQueries, PPointT *queries,
    ...
    for (IntT i = 0; i < nQueries; i++) {
        ...
        queries[i] =
            distToNN[i]
                .ppoint;  // copy the sorted queries array back to <queries>
...
void determineRTCoefficients(RealT thresholdR, RealT successProbability,
    ...
    if (algParameters.useUfunctions) {
        ...
        algParameters.parameterM = computeLfromKP(
            algParameters.parameterK, algParameters.successProbability);
    ...
    do {
        ...
        //     (RealT*)MALLOC(dimension * sizeof(RealT)))); RealT sqrLength = 0;
        ...
        for (IntT rep = 0; rep < nReps; rep++) {
            ...
            timeCycleBucket = 0;
            nOfDistComps = 0;
            nNNs = getNearNeighborsFromPRNearNeighborStruct(
...
RealT estimateNCollisions(IntT nPoints, IntT dim, PPointT *dataSet,
    ...
    for (IntT i = 0; i < nPoints; i++) {
        if (query != dataSet[i]) {
            ...
            sumCollisions +=
                POW(computeFunctionP(PARAMETER_W_DEFAULT, dist / R), k);
        }
    }
    ...
    for (IntT i = 0; i < nPoints; i++) {
        if (query != dataSet[i]) {
            ...
            if (!useUfunctions) {
                sumCollisions +=
                    1 -
                    POW(1 - POW(computeFunctionP(PARAMETER_W_DEFAULT, dist / R),
                                k),
                        LorM);
            } else {
                RealT mu =
                    1 -
                    POW(computeFunctionP(PARAMETER_W_DEFAULT, dist / R), k / 2);
...
RNNParametersT computeOptimalParameters(RealT R, RealT successProbability,
    ...
    // PPointT query = dataSet[queryIndex]; // query points = a random points
    // from the data set.
    IntT bestK = 0;
    RealT bestTime = 0;
    for (k = 2;; k += 2) {
        ...
        // dimension, dataSet, queryIndex, k, L, R);
