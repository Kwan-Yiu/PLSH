...
void sortQueryPointsByRadii(IntT dimension, Int32T nQueries, PPointT *queries,
  ...
  FAILIF(NULL == (distToNN = (PPointAndRealTStructT *)MALLOC(
                      nQueries * sizeof(*distToNN))));
  ...
  for (IntT i = 0; i < nQueries; i++) {
    // DPRINTF("%0.6lf\n", distToNN[i].real);
    queries[i] =
        distToNN[i].ppoint; // copy the sorted queries array back to <queries>
...
void determineRTCoefficients(RealT thresholdR, RealT successProbability,
  ...
  if (algParameters.useUfunctions) {
    algParameters.parameterM = computeMForULSH(
        algParameters.parameterK, algParameters.successProbability);
    algParameters.parameterL =
        algParameters.parameterM * (algParameters.parameterM - 1) / 2;
  } else {
    algParameters.parameterM = computeLfromKP(algParameters.parameterK,
                                              algParameters.successProbability);
  ...
  //     FAILIF(NULL == (dataSet[i]->coordinates = (RealT*)MALLOC(dimension *
  //     sizeof(RealT))));
  ...
  do {
    ...
    //     FAILIF(NULL == (queryPoint->coordinates = (RealT*)MALLOC(dimension *
    //     sizeof(RealT)))); RealT sqrLength = 0; for(IntT i = 0; i < dimension;
    //     i++){
    ...
    for (IntT rep = 0; rep < nReps; rep++) {
      ...
      timeCycleBucket = 0;
      nOfDistComps = 0;
      nNNs = getNearNeighborsFromPRNearNeighborStruct(nnStruct, queryPoint,
                                                      result, resultSize);
      ...
      if (nOfDistComps >= MIN(n / 10, 100)) {
        nSucReps++;
        lshPrecomp += timeComputeULSH / algParameters.parameterK /
                      algParameters.parameterM;
...
RealT computeFunctionP(RealT w, RealT c) {
  RealT x = w / c;
  return 1 - ERFC(x / M_SQRT2) -
         M_2_SQRTPI / M_SQRT2 / x * (1 - EXP(-SQR(x) / 2));
}
...
IntT computeLfromKP(IntT k, RealT successProbability) {
  return CEIL(LOG(1 - successProbability) /
              LOG(1 - POW(computeFunctionP(PARAMETER_W_DEFAULT, 1), k)));
}
...
RealT estimateNCollisionsFromDSPoint(IntT nPoints, IntT dim, PPointT *dataSet,
                                     IntT queryIndex, IntT k, IntT L, RealT R) {
  RealT sumCollisions = 0;
  for (IntT i = 0; i < nPoints; i++) {
    if (queryIndex != i) {
...
RealT estimateNDistinctCollisions(IntT nPoints, IntT dim, PPointT *dataSet,
                                  PPointT query, BooleanT useUfunctions, IntT k,
                                  IntT LorM, RealT R) {
  RealT sumCollisions = 0;
  for (IntT i = 0; i < nPoints; i++) {
    if (query != dataSet[i]) {
      ...
      if (!useUfunctions) {
        sumCollisions +=
            1 - POW(1 - POW(computeFunctionP(PARAMETER_W_DEFAULT, dist / R), k),
                    LorM);
      } else {
        RealT mu =
            1 - POW(computeFunctionP(PARAMETER_W_DEFAULT, dist / R), k / 2);
...
RealT estimateNDistinctCollisionsFromDSPoint(IntT nPoints, IntT dim,
                                             PPointT *dataSet, IntT queryIndex,
                                             BooleanT useUfunctions, IntT k,
                                             IntT LorM, RealT R) {
  RealT sumCollisions = 0;
  for (IntT i = 0; i < nPoints; i++) {
    if (queryIndex != i) {
...
RNNParametersT computeOptimalParameters(RealT R, RealT successProbability,
  ...
  // Compute the run-time parameters (timings of different parts of the
  // algorithm).
  ...
  for (IntT i = 0; i < nReps; i++) {
    ...
    determineRTCoefficients(
        optParameters.parameterR, optParameters.successProbability,
        optParameters.useUfunctions, optParameters.typeHT,
        optParameters.dimension, nPoints, dataSet, lP, uO, dC);
    lshPrecomp += lP;
    uhashOver += uO;
    distComp += dC;
  ...
  // PPointT query = dataSet[queryIndex]; // query points = a random points from
  // the data set.
  IntT bestK = 0;
  RealT bestTime = 0;
  for (k = 2;; k += 2) {
    ...
    timeUH = L * uhashOver;
    // RealT nCollisions = estimateNCollisionsFromDSPoint(nPoints, dimension,
    // dataSet, queryIndex, k, L, R);

    // Compute the mean number of collisions for the points from the sample
    // query set.
    RealT nCollisions = 0;
    for (IntT i = 0; i < nSampleQueries; i++) {
      nCollisions += estimateNDistinctCollisions(
          nPoints, dimension, dataSet, sampleQueries[i], TRUE, k, m, R);
    }
  ...
  // Compute the mean number of collisions for the points from the sample query
  // set.
  RealT nCollisions = 0;
  for (IntT i = 0; i < nSampleQueries; i++) {
    nCollisions += estimateNDistinctCollisions(nPoints, dimension, dataSet,
                                               sampleQueries[i], TRUE, k, m, R);
  }
  nCollisions /= nSampleQueries;

  // timeCycling = estimateNCollisionsFromDSPoint(nPoints, dimension, dataSet,
  // queryIndex, bestK, L, R) * distComp;
  timeCycling = nCollisions * distComp;
