...
void usage(char *programName) {
    printf(
        "Usage: %s #pts_in_data_set #queries dimension successProbability "
...
int main(int nargs, char **args) {
    ...
    if (thresholdR == 0 || endPtr[1] == args[5]) {
        ...
        FAILIF(NULL == (memRatiosForNNStructs =
                            (RealT *)MALLOC(nRadii * sizeof(RealT))));
    ...
    if (nargs > 9) {
        ...
        if (strcmp("-c", args[9]) == 0) {
            ...
            for (IntT i = 0; i < nRadii; i++) {
                ...
                Int32T segregatedQStart =
                    (i == 0) ? 0 : sampleQBoundaryIndeces[i - 1];
                ...
                RNNParametersT optParameters = computeOptimalParameters(
                    listOfRadii[i], successProbability, nPoints,
                    pointsDimension, dataSetPoints, segregatedQNumber,
            ...
            for (IntT i = 0; i < nRadii; i++) {
                ...
                nnStructs[i] = initLSH_WithDataSet(algParameters[i], nPoints,
                                                   dataSetPoints);
            }
            ...
            FREE(listOfRadii);
            FAILIF(NULL ==
                   (listOfRadii = (RealT *)MALLOC(nRadii * sizeof(RealT))));
            for (IntT i = 0; i < nRadii; i++) {
        ...
        FAILIF(NULL == (nnStructs = (PRNearNeighborStructT *)MALLOC(
                            nRadii * sizeof(PRNearNeighborStructT))));
        // Determine the R-NN DS parameters, construct the DS and run the
        // queries.
        transformMemRatios();
        for (IntT i = 0; i < nRadii; i++) {
    ...
    for (IntT i = 0; i < nQueries; i++) {
        ...
        for (IntT r = 0; r < nRadii; r++) {
            nNNs =
                getRNearNeighbors(nnStructs[r], queryPoint, result, resultSize);
            printf(
                "Total time for R-NN query at radius %0.6lf (radius no. "
                "%d):\t%0.6lf\n",
            ...
            if (nNNs > 0) {
                printf(
                    "Query point %d: found %d NNs at distance %0.6lf (%dth "
                    "radius). "
                    "First %d NNs are:\n",
                    i, nNNs, (double)(listOfRadii[r]), r,
                    MIN(nNNs, MAX_REPORTED_POINTS));

                // compute the distances to the found NN, and sort according to
                // the distance
                ...
                for (IntT p = 0; p < nNNs; p++) {
                    ...
                    distToNN[p].real =
                        distance(pointsDimension, queryPoint, result[p]);
                }
                qsort(distToNN, nNNs, sizeof(*distToNN),
                      comparePPointAndRealTStructT);
                ...
                for (IntT j = 0; j < MIN(nNNs, MAX_REPORTED_POINTS); j++) {
                    ...
                    // DPRINTF("Distance: %lf\n", distance(pointsDimension,
                    // queryPoint, result[j])); printRealVector("NN: ",
                    // pointsDimension, result[j]->coordinates);
                }
                break;
            }
        }
