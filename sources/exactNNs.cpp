...
void usage(char *programName) {
    printf(
        ...
        "radius data_set_file queries_file\n",
        programName);
}
...
// Returns 1 iff the square of the Euclidean distance from point <p1> to <p2> is
// <=threshold.
    ...
    for (int i = 0; i < nQueries; i++) {
        ...
        for (int r = 0; r < nRadii; r++) {
            ...
            for (int j = 0; j < nPoints; j++) {
                ...
                // points[j])); printRealVector1("X: ", dimension, points[j]);
            }
            TIMEV_END(time);  // time only finding the near neighbors, and
                              // exclude printing from timing.
            printf(
            ...
            if (nNNs > 0) {
                printf(
                    "Query point %d: found %d NNs at distance %0.6lf (radius "
                    "no. "
                    "%d). NNs are:\n",
                    i, nNNs, (double)(listOfRadii[r]), r);
                for (int j = 0; j < nNNs; j++) {
                    printf("%09d\n", nearNeighbors[j]);
                    // printRealVector1("NN: ", dimension,
                    // points[nearNeighbors[j]]);
                }
                break;
            }
        }
