...
float calculate_angular_distance(const SparseVector& v1, const SparseVector& v2) {
...
std::vector<Result> find_ground_truth(
...
int main() {
    std::cout << "--- PLSH Index Full Demo with Streaming & Periodic Merge ---" << std::endl;
    ...
    std::cout << "\n[CONFIG]" << std::endl;
    std::cout << "  - Dimensions: " << dimensions << ", k: " << k << ", m: " << m << std::endl;
    std::cout << "  - Threads: " << num_threads << std::endl;
    std::cout << "  - Data Size: " << initial_points << " (initial) + " << streaming_points << " (streaming) = " << total_points << std::endl;
    std::cout << "  - Merge Threshold: " << merge_threshold << " inserts" << std::endl;
    ...
    try {
        ...
        std::vector<SparseVector> initial_data(all_data.begin(), all_data.begin() + initial_points);
        std::vector<SparseVector> streaming_data(all_data.begin() + initial_points, all_data.end());
        std::cout << "  - Generated " << total_points << " sparse vectors." << std::endl;
        ...
        std::cout << "  - Built initial index with " << initial_points << " points in " << build_duration.count() << "s." << std::endl;
        ...
        for (const auto& point : streaming_data) {
            ...
            if (insert_count_since_merge == merge_threshold) {
                std::cout << "  -> Reached insert threshold. Triggering index rebuild..." << std::endl;
                ...
                std::chrono::duration<double> rebuild_duration = rebuild_end - rebuild_start;
                std::cout << "  -> Rebuild finished in " << rebuild_duration.count() << "s." << std::endl;
                insert_count_since_merge = 0; 
            }
        }
        
        if (insert_count_since_merge > 0) {
             std::cout << "  -> Performing final merge for remaining " << insert_count_since_merge << " points..." << std::endl;
             index.merge_delta_to_static();
        }
        ...
        double effective_insert_qps = streaming_points / streaming_duration.count();
        std::cout << std::fixed << std::setprecision(2);
        std::cout << "  - Total streaming inserts: " << streaming_points << std::endl;
        std::cout << "  - Total streaming time (inserts + merges): " << streaming_duration.count() << " s" << std::endl;
        std::cout << "  - Effective Insert QPS: " << effective_insert_qps << " ops/sec" << std::endl;

        std::cout << "\n[PHASE 4: Query Performance Test (After all merges)]" << std::endl;
        ...
        std::cout << "  - Total Time:    " << query_duration.count() << " s" << std::endl;
        std::cout << "  - Query QPS:     " << query_qps << " ops/sec" << std::endl;
        std::cout << "  - Avg Latency:   " << avg_latency_ms << " ms/query" << std::endl;

        std::cout << "\n[PHASE 5: Recall Verification (for a single query)]" << std::endl;
        ...
        std::vector<Result> ground_truth = find_ground_truth(query_point, all_data, radius, query_idx_for_recall);
        ...
        double recall = ground_truth.empty() ? 1.0 : static_cast<double>(true_positives) / ground_truth.size();
        std::cout << "  - Query Point ID: " << query_idx_for_recall << std::endl;
        std::cout << "  - Ground Truth Neighbors (within radius " << radius << "): " << ground_truth.size() << std::endl;
        std::cout << "  - LSH Found Neighbors (within radius " << radius << "): " << lsh_results.size() << std::endl;
        std::cout << "  - Correctly Found (True Positives): " << true_positives << std::endl;
