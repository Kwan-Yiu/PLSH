...
float calculate_angular_distance_dense(const DenseVector& v1, const DenseVector& v2) {
...
std::vector<Result> find_ground_truth_dense(
    const DenseVector& query_point,
    const std::vector<DenseVector>& all_data,
    float radius,
    uint32_t query_id_to_exclude) 
{
    ...
    for (uint32_t i = 0; i < all_data.size(); ++i) {
        ...
        float distance = calculate_angular_distance_dense(query_point, all_data[i]);
        if (distance <= radius) {
...
int main() {
    std::cout << "--- PLSH Index Demo Adapted for DENSE Vectors (e.g., SIFT-like data) ---" << std::endl;
    ...
    try {
        ...
        std::cout << "  - Generated " << total_points << " dense vectors." << std::endl;
        ...
        for(int i = 0; i < initial_points; ++i) {
            initial_data_sparse.push_back(dense_to_sparse_and_normalize(all_data[i]));
        }
        index.build(initial_data_sparse);
        std::cout << "  - Built initial index with " << initial_points << " points." << std::endl;
        ...
        SparseVector query_point_sparse = dense_to_sparse_and_normalize(query_point_dense);
        std::vector<Result> lsh_results = index.query(query_point_sparse, radius);
        std::vector<Result> ground_truth = find_ground_truth_dense(query_point_dense, all_data, radius, query_idx_for_recall);
        ...
        std::cout << "  - Ground Truth Neighbors: " << ground_truth.size() << std::endl;
        std::cout << "  - LSH Found Neighbors:    " << lsh_results.size() << std::endl;
        std::cout << "  - Correctly Found:      " << true_positives << std::endl;
